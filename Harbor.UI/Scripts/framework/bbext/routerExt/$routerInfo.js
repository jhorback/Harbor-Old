  ...
  *         *path - The root path.
  */
bbext.routerInfo = function (appurl, console, globalCache) {

	var meta = globalCache.track("bbext.routerInfo.meta", {
		root: appurl.get(""),
		rootSet: false,
		urls: {}
	});
	...
	return {
		...
		setRoot: function (path) {
		    var newRoot = appurl.get(path),
                locationRootSegment = location.pathname.substring(0, newRoot.length - 1) + "/";

		    // match the current location bar case
            // this in essence makes the "root" case insensitive
		    if (newRoot.toLowerCase() === locationRootSegment.toLowerCase()) {
		        newRoot = locationRootSegment;
		    }

			if (meta.rootSet && newRoot !== meta.root) {
				console.error("Changing page root from '" + meta.root + "' to '" + newRoot + "'");
			}
			meta.root = newRoot;
			meta.rootSet = true;
			return meta.root;
