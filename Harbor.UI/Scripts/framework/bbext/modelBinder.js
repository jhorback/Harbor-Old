 ...
 *
 *     Event overriding:
 *     1) data-bind-event="eventName"; this also opts out of autofill corrections.
 * 
 *     Autofill corrections
 *     Visible text, password, and select fields have their value polled once after page load
 *     to see if the browser filled in any values. Specify the binding event directly to
 *     opt out of this behavior (data-bind-event="change").
 *
...
var modelBinder = function ($, _, config, nameValueParser) {
	...
	function ModelBinder(model, el, matches) {
		...
		this.$el = $(el);
		matches = matches || el.find("[data-bind]:not([data-view-from]),[name],[id]").addBack("[data-bind]");
		if (matches.length === 0) {
			return this;
		}
			...
			update = function () {
				...
				_.each(self.bindings, function (binding, propName) {
				    _.each(binding.els, function (el) {
						if (el.not("[data-bind-event]").length !== 0 && el.is(":text,:password,select") && el.is(":visible")) {
